      ...
      <Container className="relative z-10">
        ...
        >
          {contattiContent.needs.blocks.map((block, index) => (
            <RevealItem key={block.title} preset="fade-up" distance={36} duration={1.15}>
              <article
              ...
              >
                <RevealGroup className="flex h-full flex-col gap-y-4" stagger={0.08}>
                  <RevealItem preset="fade-up" distance={16} duration={0.85}>
                    <h3 id={`contact-need-title-${index}`} className="text-3xl font-black text-lq-dark">
                      {block.title}
                    </h3>
                  </RevealItem>
                  <RevealItem preset="fade-up" distance={20} duration={0.9}>
                    <p id={`contact-need-description-${index}`} className="text-base font-normal text-lq-dark/90">
                      {block.description}
                    </p>
                  </RevealItem>
