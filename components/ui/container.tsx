...
function Container({ children, className }: Readonly<{ children: React.ReactNode; className?: string }>) {
  return (
    <div slot="container" className={cn('container mx-auto max-w-7xl px-8 lg:px-10 xl:px-20', className)}>
      {children}
    </div>
  )
}
...
function DiagonalSection({
  ...
  return (
    ...
    >
      <Container className={cn('relative z-10 pt-24 pb-20', containerClassName)}>{children}</Container>
      <div
        aria-hidden
        className={cn(
