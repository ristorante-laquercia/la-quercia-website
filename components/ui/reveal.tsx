...
function getItemVariants(
  ...
  switch (preset) {
    case "fade-down":
      return {
        hidden: { opacity: 0, y: addAxisDistance(arriveY, -distance), x: targetX },
        visible: {
          opacity: 1,
          y: targetY,
          x: targetX,
    ...
    case "fade-left":
      return {
        hidden: { opacity: 0, x: addAxisDistance(arriveX, distance), y: targetY },
        visible: {
          opacity: 1,
          x: targetX,
          y: targetY,
...
function RevealItem({
  ...
  return (
    <motion.div
      className={cn(className)}
      variants={getItemVariants(preset, distance, duration, arriveX, arriveY)}
    >
      {children}
    </motion.div>
  );
